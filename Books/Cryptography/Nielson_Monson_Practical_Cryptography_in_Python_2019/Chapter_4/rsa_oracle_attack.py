...
def int_to_bytes(i: int, min_size=None):
...
# RSA Oracle Attack Component, part of class RSAOracleAttacker 
    def _step2a_start_the_searching(self):
        si = self._find_s(start_s=math.ceil(self.n / (3*self.B)))
        return si
...
# RSA Oracle Attack Component, part of class RSAOracleAttacker 
    def _step2c_searching_with_one_interval_left(self):
        a,b = self.M[-1][0]
        ri = math.ceil(2*(b*self.s[-1] - 2*self.B) / self.n)
        si = None
    
        while si == None:
            si = math.ceil((2*self.B+ri*self.n) / b)
            
            s_max = math.ceil((3*self.B+ri*self.n) / a)
...
# RSA Oracle Attack Component, part of class RSAOracleAttacker
    def _step3_narrowing_set_of_solutions(self, si):
        new_intervals = set()
        for a,b in self.M[-1]:
            r_min = math.ceil((a*si - 3*self.B + 1) / self.n)
            r_max = math.floor((b*si - 2*self.B) / self.n)

            for r in range(r_min, r_max+1):
                a_candidate = math.ceil((2*self.B+r*self.n) / si)
                b_candidate = math.floor((3*self.B-1+r*self.n) / si)
...
def main(args):
    ...
    print(f"Running {messagecount} tests with key size {key_size}")
    ...
    for i in range(messagecount):
        ...
        print(f"\nWe're starting our attack run on message {i}.")
